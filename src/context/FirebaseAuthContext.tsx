...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const defaultUsers: { [key: string]: Omit<User, 'id'> } = {
    'inmobiliariaathouse@gmail.com': { username: 'admin', role: 'admin', email: 'inmobiliariaathouse@gmail.com' },
  ...
  const updateClientMigration = async (clientId: string, migration: ClientMigrationState): Promise<void> => {
    ...
    if (!clientSnapshot.exists() || clientSnapshot.data().userId !== firebaseUser.uid) {
      throw new Error('El cliente no existe o no pertenece al usuario actual.');
    }
    if (!isLegacyMigrationEligible(clientSnapshot.data(), CURRENT_PAYMENT_SCHEDULE_VERSION)) {
